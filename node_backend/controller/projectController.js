...
const listByStatus = asyncHandler(async (req, res) => {
  try {
    ...
    if (projectStatus === "All") {
      ...
      const projectList = await Project.find({ projectStatus })
        .sort("-_id")
        .populate("companyId");
    ...
    // let updatedAyurvedasList = []
    // for(let i = 0i < ayurvedasList.length i++)
    // {
    //     console.log(ayurvedasList[i].diseaseId.name)
    // }

    // res.send({
    //     ayurvedasList : updatedAyurvedasList,
    //     success : true,
    //     status : 200
    // })
  } catch (error) {
...
// const listByStatus = asyncHandler(async (req, resp) => {
//   resp.send({})
// })
